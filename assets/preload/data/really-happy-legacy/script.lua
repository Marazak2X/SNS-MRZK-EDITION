function onCreate()
   ...
   addCharacterToList('mouse-reanimated', 'dad');
   precacheImage('suicideUI/NOTE_assets');
...
function onStartCountdown()
   if not allowCountdown and getPropertyFromClass('ClientPrefs','newPlayer') then
...
function onStepHit()
   ...
   if curSection >= 97 and curSection <= 100 then
      if curStep % 1 == 0 then
         triggerEvent('Screen Shake', '0.2,0.015', '0.2,0.015');
      end
   end

   if curStep >= 896 and curStep <= 905 then
      if curStep % 1 == 0 then
         triggerEvent('Screen Shake', '0.2,0.015', '0.2,0.015');
      end
   end
...
function opponentNoteHit()
   ...
   if getProperty('dad.curCharacter') == 'mouse-reanimated' then
      ...
      if getProperty('health') > 0.2 then
         runHaxeCode([[
            var healthTween = FlxTween.tween(game, {health: game.health - 0.021}, 0.15, {ease: FlxEase.circOut, onComplete: function(twn:FlxTween)
               {
                  healthTween = null;
               }
            });
